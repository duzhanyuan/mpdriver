...
func (mrs *MysqlResultSet) Columns() ([]*ColumnInfo, error) {
	return mrs.columns, nil
}

func (mrs *MysqlResultSet) Next() (row []types.Datum, err error) {
	if mrs.cursor == len(mrs.rows) {
		return nil, nil
	}
	row = mrs.rows[mrs.cursor]
	mrs.cursor++
	return
}

func (mrs *MysqlResultSet) Close() error {
	mrs.cursor = 0
	return nil
}

func (md *MysqlDriver) OpenCtx(connectionId uint64, capability uint32, collation uint8, dbname string) (ctx IContext, err error) {
	mc := new(MysqlConn)
	mc.stmts = make(map[int]*MysqlStatement)
	mc.capability = capability & defaultCapability
	log.Debugf("client id:%d, capabiliy: %0.32b, collation: %d, db: %s", connectionId, capability, collation, dbname)
	mc.collation = collation
	mc.connID = connectionId
...
func (ms *MysqlStatement) Execute(args ...interface{}) (rs ResultSet, err error) {
	log.Debugf("(%d)EXEC: %s [%v]", ms.mConn.connID, ms.sql, args)
	WriteCmdRecord(ms.mConn.connID, ms.sql, args...)
	ms.Reset()
...
func (ms *MysqlStatement) AppendParam(paramId int, data []byte) (err error) {
	if paramId >= len(ms.boundParams) {
		return NewErr(ErrWrongArguments, "stmt_send_longdata")
	}
...
func (mc *MysqlConn) exec(query string) (ResultSet, error) {
	...
	WriteCmdRecord(mc.connID, query)
	log.Debugf("(%d)QUERY: %s", mc.connID, query)
...
func (mc *MysqlConn) readResultRows(result *MysqlResultSet, isBinary bool) (err error) {
	var data []byte
	var rowDatas [][]byte
	for {
		data, err = mc.readPacket()
	...
	result.rows = make([][]types.Datum, len(rowDatas))
...
func (mc *MysqlConn) Prepare(query string) (stmt IStatement, columns, params []*ColumnInfo, err error) {
	log.Debugf("(%d)PREPARE: %s", mc.connID, query)
...
func (mc *MysqlConn) Auth(user string, auth []byte, salt []byte) bool {
	return true
}
